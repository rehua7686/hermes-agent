...
def _resolve_base_url() -> str:
    """Resolve the DeepInfra base URL (env > config > default)."""
    env = os.environ.get("DEEPINFRA_BASE_URL", "").strip()
    if env:
        return env.rstrip("/")
    cfg = _load_deepinfra_image_config()
    val = cfg.get("base_url") if isinstance(cfg.get("base_url"), str) else None
    if val:
        return val.rstrip("/")
    return _DEEPINFRA_DEFAULT_BASE_URL
...
class DeepInfraImageGenProvider(ImageGenProvider):
    ...
    def generate(
        ...
        **kwargs: Any,
        ...
        model_id = _resolve_model(catalog)
        if not model_id:
            return error_response(
                error=(
        ...
        if b64:
            try:
                # Drop the ``vendor/`` prefix and any colons so the saved
                # filename stays a single path component on every OS.
