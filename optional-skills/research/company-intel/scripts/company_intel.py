...
def url_encode(params: dict) -> str:
    ...
    return "&".join(f"{k}={urllib.parse.quote(str(v), safe='')}" for k, v in params.items() if v is not None)
...
def wikipedia_search(query: str, num: int = 5) -> dict:
    ...
    return {"query": query, "total": data.get("query", {}).get("searchinfo", {}).get("totalhits", 0), "results": results}
...
def wikidata_resolve_uris(uris: list) -> dict:
    ...
    if not uris:
        return {}
    ids = [u.split("/")[-1] for u in uris if u.startswith("http://www.wikidata.org/entity/")]
    if not ids:
        return {}
...
def wikidata_query(company_name: str) -> dict:
    ...
    try:
        data = api_request(url, {"Accept": "application/json", "User-Agent": USER_AGENT})
        ...
        data2 = api_request(url2, {"Accept": "application/json", "User-Agent": USER_AGENT})
...
def cmd_profile(args):
    ...
    if "error" in wiki and "error" in wd:
        output["error"] = f"Could not find company '{args.company}' in Wikipedia or Wikidata"
...
def main():
    ...
    if command == "profile":
        import argparse
        p = argparse.ArgumentParser(description="Get company profile (Wikipedia + Wikidata)")
        ...
        p.add_argument("company", help="Company name")
        p.add_argument("--days", type=int, default=7, help="How many days back to search")
        args = p.parse_args(sys.argv[2:])
        cmd_news(args)
