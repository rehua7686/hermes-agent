---
name: claude-import
description: Import Claude conversations. Needs an external memory provider.
version: 1.0.0
...
for i, batch in enumerate(batches):
    # Combine batch into one memory entry
    combined = "\n\n---\n\n".join(
        u["content"] for u in batch
    )
...
```bash
# Check the provider's memory count
hermes memory status
# or: hermes doctor

# Search for an imported conversation by name
memory search "conversation_name"
```
