...
export async function exportSession(sessionId: string, params: Omit<ExportSessionParams, 'sessionId'> = {}) {
  ...
  try {
    ...
    notify({ kind: 'success', message: translateNow('chat.notifications.sessionExported'), durationMs: 2_000 })
  } catch (err) {
    notifyError(err, translateNow('chat.notifications.sessionExportFailed'))
  }
}
