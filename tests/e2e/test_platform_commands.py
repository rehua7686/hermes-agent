...
class TestSlashCommands:
    ...
    @pytest.mark.asyncio
    async def test_notebooklm_without_args_returns_usage(self, adapter, runner, platform):
    ...
    @pytest.mark.asyncio
    async def test_notebooklm_with_url_falls_through_to_agent(self, adapter, runner, platform):
        send = await send_and_capture(adapter, "/notebooklm url https://example.com/a", platform)
        ...
        assert "Run the NotebookLM LearnPack workflow for: url https://example.com/a" in event.text

    @pytest.mark.asyncio
    async def test_nlm_alias_preserves_chinese_payload(self, adapter, runner, platform):
        send = await send_and_capture(adapter, "/nlm 知识库里的 agent memory", platform)
