...
def _make_provider(search_results=None, config_overrides=None):
    """Return an initialised Mem0MemoryProvider with a fake client."""
    fake_mem0 = ModuleType("mem0")
    fake_client = MagicMock()
    results_payload = search_results if search_results is not None else {
        "results": [{"memory": "user likes dark mode"}]
    }
    fake_client.search.return_value = results_payload
    fake_mem0.MemoryClient = MagicMock(return_value=fake_client)
