...
* default 1 retry (configurable via ``HERMES_VISION_AUTO_RETRIES``)
...
# ---------------------------------------------------------------------------
# _vision_auto_retry_count — env var resolution
# ---------------------------------------------------------------------------


class TestVisionAutoRetryCountResolver:
    def test_default_when_env_unset(self, monkeypatch, gateway_runner):
        monkeypatch.delenv("HERMES_VISION_AUTO_RETRIES", raising=False)
    ...
    def test_negative_clamped_to_zero(self, monkeypatch, gateway_runner):
        ...
        monkeypatch.setenv("HERMES_VISION_AUTO_RETRIES", "-5")
        assert gateway_runner._vision_auto_retry_count() == 0

    def test_unparsable_falls_back_to_default(self, monkeypatch, gateway_runner):
        # Typo in the env var must NOT silently disable the safety
        # net (which would re-introduce the very bug we're fixing).
        monkeypatch.setenv("HERMES_VISION_AUTO_RETRIES", "off")
        assert gateway_runner._vision_auto_retry_count() == 1

    def test_whitespace_tolerated(self, monkeypatch, gateway_runner):
        monkeypatch.setenv("HERMES_VISION_AUTO_RETRIES", "  2  ")
...
class TestVisionAnalyzeAutoRetry:
    ...
    def test_retries_disabled_via_env(self, gateway_runner, monkeypatch):
