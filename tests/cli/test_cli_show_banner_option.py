...
def _patch_banner_calls(cli_mod):
    ...
    """
    with patch.object(cli_mod, "build_welcome_banner") as mock_banner, patch.object(
        cli_mod, "_build_compact_banner"
    ) as mock_compact, patch.object(
        cli_mod, "get_tool_definitions", return_value=[]
    ), patch.object(
        cli_mod.shutil, "get_terminal_size", return_value=os.terminal_size((120, 40))
    ):
