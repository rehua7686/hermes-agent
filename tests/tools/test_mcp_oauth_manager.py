...
@pytest.mark.asyncio
async def test_hermes_provider_accepts_supabase_2xx_token_response(
    tmp_path, monkeypatch, status_code
):
    """Supabase token exchange can return non-200 2xx with valid token JSON."""
    monkeypatch.setenv("HERMES_HOME", str(tmp_path))
    import httpx
    from tools.mcp_oauth import HermesTokenStorage
    from tools.mcp_oauth_manager import _HERMES_PROVIDER_CLS
...
@pytest.mark.parametrize("status_code", [201, 202])
@pytest.mark.asyncio
async def test_hermes_provider_accepts_supabase_2xx_refresh_response(
    tmp_path, monkeypatch, status_code
):
    """Supabase refresh can return non-200 2xx with valid token JSON."""
    monkeypatch.setenv("HERMES_HOME", str(tmp_path))
    import httpx
    from tools.mcp_oauth import HermesTokenStorage
    from tools.mcp_oauth_manager import _HERMES_PROVIDER_CLS
