...
class TestSendMessageTool:
    ...
    def test_trigger_agent_send_reports_trigger_error_without_live_runner(self):
        config, telegram_cfg = _make_config()
    ...
    def test_trigger_gateway_agent_reports_missing_adapter(self):
        ...
        with patch("gateway.run._gateway_runner_ref", return_value=runner):
            result = asyncio.run(_trigger_gateway_agent("telegram", "12345", "wake hello"))

        assert result == {"trigger_error": "no live adapter for telegram"}

    def test_trigger_gateway_agent_reports_missing_event_loop(self):
        runner = SimpleNamespace(
        ...
        assert result == {"trigger_error": "no live gateway event loop"}
