...
def _make_hermes_provider_class() -> Optional[type]:
    ...
    class HermesMCPOAuthProvider(OAuthClientProvider):
        ...
        async def _handle_token_response(self, response):  # type: ignore[override]
            """Handle OAuth token responses, accepting valid non-200 2xx.
            ...
            authorization-code exchange. Treat valid non-200 2xx token bodies as
            success instead of forcing a second browser OAuth loop.
            """
        ...
        async def _handle_refresh_response(self, response):  # type: ignore[override]
            """Handle OAuth refresh responses, accepting valid non-200 2xx.
