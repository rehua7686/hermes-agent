...
| `HERMES_TERMINAL_WRITE_GUARD` | `off`/`warn`/`block`——将 `HERMES_WRITE_SAFE_ROOT` 扩展到 `terminal`/`execute_code`（尽力而为的静态扫描）。`warn`（默认）会向模型标记越界写入；`block` 则直接拒绝。未设置 `HERMES_WRITE_SAFE_ROOT` 时无效。属纵深防御，并非安全边界。 |
