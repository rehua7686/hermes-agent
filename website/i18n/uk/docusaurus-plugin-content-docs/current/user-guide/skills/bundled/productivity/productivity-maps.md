---
title: "Карти — геокодування, POI, маршрути, часові пояси через OpenStreetMap/OSRM"
sidebar_label: "Maps"
description: "Геокодування, POIs, маршрути, часові пояси via OpenStreetMap/OSRM"
---
...
- Користувач надсилає Telegram‑пін локації (широта/довгота в повідомленні) → `nearby`
- Користувач хоче координати для назви місця → `search`
- Користувач має координати і хоче адресу → `reverse`
- Користувач запитує ресторани, лікарні, аптеки, готелі тощо поблизу → `nearby`
- Користувач хоче відстань або час у дорозі для авто/пешої/велосипедної поїздки → `distance`
- Користувач хоче покрокові інструкції між двома місцями → `directions`
- Користувач хоче інформацію про часовий пояс для локації → `timezone`
- Користувач хоче шукати POI у географічній області → `area` + `bbox`
...
### search — Геокодування назви місця
...
### nearby — Пошук місць за категорією
...
### distance — Відстань та час у дорозі
...
### directions — Покрокова навігація
...
### timezone — Часовий пояс для координат
...
### area — Обмежувальний прямокутник і площа для місця
...
### bbox — Пошук у межах обмежувального прямокутника
...
## Робота з Telegram‑піном локації

Коли користувач надсилає пін локації, у повідомленні присутні поля `latitude:` і
`longitude:`. Витягни їх і передай безпосередньо до `nearby`:
