...
description: "Витягувати структуровані дані з відповідей LLM за допомогою валідації Pydantic, автоматично повторювати невдалі витяги, парсити складний JSON із забезпеченням типобезпеки та потоково ..."
---
...
Витягай структуровані дані з відповідей LLM за допомогою валідації Pydantic, автоматично повторюй невдалі витяги, парси складний JSON із типобезпекою та транслюй часткові результати за допомогою Instructor — випробувана бібліотека для структурованого виводу
...
# Instructor: Структуровані виводи LLM

## Коли використовувати цю навичку

Використовуй Instructor, коли потрібно:
- **Витягати структуровані дані** з відповідей LLM надійно
- **Валідувати вивід** проти схем Pydantic автоматично
- **Повторювати невдалі витяги** з автоматичною обробкою помилок
- **Парсити складний JSON** з типобезпекою та валідацією
- **Транслювати часткові результати** для обробки в реальному часі
- **Підтримувати кілька провайдерів LLM** з уніфікованим API
...
### Основний приклад: витяг користувацьких даних
...
### 1. Моделі відповіді (Pydantic)

Моделі відповіді визначають структуру та правила валідації для виводу LLM.
...
**Переваги:**
- Типобезпека з підказками типів Python
- Автоматична валідація (word_count > 0)
- Самодокументування за допомогою описів `Field`
- Підтримка автодоповнення в IDE
...
Pydantic автоматично валідовує вивід LLM. Якщо валідація не проходить, Instructor повторює.
...
### 3. Автоматичне повторення

Instructor автоматично повторює, коли валідація не проходить, передаючи повідомлення про помилку LLM.
...
**Як це працює:**
1. LLM генерує вивід
2. Pydantic валідовує
3. Якщо недійсний — повідомлення про помилку надсилається назад LLM
4. LLM пробує ще раз з урахуванням помилки
5. Повторюється до `max_retries`

### 4. Трансляція

Транслюй часткові результати для обробки в реальному часі.

#### Трансляція часткових об’єктів
...
#### Трансляція ітерованих об’єктів
...
### Шаблон 3: Витяг кількох сутностей
...
### Шаблон 4: Структурований аналіз
...
### 2. Використовуй відповідну валідацію
...
### 3. Надавай приклади в підказках
...
### 4. Використовуй перелічення для фіксованих категорій
...
### 5. Обробляй відсутні дані коректно
...
**Коли обирати Instructor:**
- Потрібні структуровані, валідовані виводи
- Потрібна типобезпека та підтримка IDE
- Потрібні автоматичні повторення
- Будуєш системи витягу даних

**Коли обирати альтернативи:**
- DSPy — потрібна оптимізація підказок
- LangChain — будівництво складних ланцюжків
- Manual — простий, одноразовий витяг
