---
title: "Fastmcp — створюй, тестуй, інспектуй, встановлюй та розгортай сервери MCP за допомогою FastMCP на Python"
sidebar_label: "Fastmcp"
description: "Створюй, тестуй, перевіряй, встановлюй та розгортай сервери MCP за допомогою FastMCP на Python"
---
...
Будуй, тестуй, інспектуй, встановлюй та розгортай MCP‑сервери за допомогою FastMCP на Python. Використовуй, коли створюєш новий MCP‑сервер, обгортаєш API або базу даних у MCP‑інструменти, експонуєш ресурси чи підказки, або готуєш FastMCP‑сервер для Claude Code, Cursor чи HTTP‑розгортання.
...
Будуй MCP‑сервери на Python за допомогою FastMCP, валідуй їх локально, встановлюй у MCP‑клієнти та розгортай як HTTP‑ендпоінти.

## When to Use

Використовуй цей інструмент, коли завдання полягає у:

- створенні нового MCP‑серверу на Python
- обгортанні API, бази даних, CLI або робочого процесу обробки файлів у MCP‑інструменти
- експонуванні ресурсів або підказок разом з інструментами
- smoke‑тестуванні серверу за допомогою FastMCP CLI перед підключенням його до Hermes або іншого клієнта
- встановленні серверу у Claude Code, Claude Desktop, Cursor або подібний MCP‑клієнт
- підготовці репозиторію FastMCP‑серверу для HTTP‑розгортання

Використовуй `native-mcp`, коли сервер вже існує і потрібно лише підключити його до Hermes. Використовуй `mcporter`, коли мета — ad‑hoc CLI‑доступ до існуючого MCP‑серверу замість його створення.

## Prerequisites

Встанови FastMCP у робочому середовищі спочатку:
...
Для шаблону API встанови `httpx`, якщо він ще не встановлений:
...
Вибери найвужчу корисну поверхню спочатку:
