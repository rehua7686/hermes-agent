...
description: "Часто задаваемые вопросы и решения распространённых проблем с Hermes Agent"
---
...
Hermes auto‑detects local endpoints and relaxes streaming timeouts (read timeout raised from 120 s to 1800 s, stale‑stream detection disabled). If you still hit timeouts on very large contexts, set `HERMES_STREAM_READ_TIMEOUT=1800` in your `.env`. See the [Local LLM guide](../guides/local-llm-on-mac.md#timeouts) for details.
:::
...
Missing files are skipped silently. Sourcing happens in bash, so files that rely on zsh‑only syntax may error — if that's a concern, source just the PATH‑setting portion (e.g. `nvm.sh`) rather than the whole rc file.
...
| Switch to a different configured provider | `/model provider:model` (inside session) |
:::
...
- If you must use `sudo`, configure password‑less sudo for specific commands in `/etc/sudoers`
...
**Cause:** launchd services inherit a minimal `PATH` (`/usr/bin:/bin:/usr/sbin:/sbin`), which doesn't include Homebrew, nvm, cargo, or other user‑installed tool directories. This commonly breaks the WhatsApp bridge (`node not found`) or voice transcription (`ffmpeg not found`).
...
**Solution:** Delegation config. Hermes can route sub‑agents to a different model automatically. Add to `~/.hermes/config.yaml`:
...
Now when you tell Hermes “write me a Twitter thread about X” and it spawns a `delegate_task` sub‑agent, that sub‑agent runs on Gemini instead of your main model. Your primary conversation stays on GPT‑5.4.

You can also be explicit in your prompt: *“Delegate a task to write social media posts about our product launch. Use your sub‑agent for the actual writing.”* The agent will use `delegate_task`, which automatically picks up the delegation config.
...
1. **One profile with personality switching.** Create different `AGENTS.md` context files or use the `/personality` command to change behavior per chat. The agent sees which chat it's in and can adapt.
2. **Cron jobs for specialized tasks.** For a shopping‑list tracker, set up a cron job that monitors a specific chat and manages the list — no separate agent needed.
3. **Separate numbers.** If you need truly independent agents, pair each profile with its own WhatsApp number. Virtual numbers from services like Google Voice work for this.
4. **Telegram or Discord instead.** These platforms naturally support per‑chat binding — each Telegram group or Discord channel gets its own session, and you can run multiple bot tokens (one per profile) on the same account.

See [Profiles](../user-guide/profiles.md) and [WhatsApp setup](../user-guide/messaging/whatsapp.md) for details.
...
:::tip
Skills with very long descriptions are truncated to 40 characters in the Telegram menu to stay within payload size limits. If skills aren't appearing, it may be a total payload size issue rather than the 100‑command limit — disabling unused skills helps with both.
:::
...
1. **Use Slack.** Slack sessions are keyed by thread, not by user. Multiple users in the same thread share one conversation — exactly the behavior you need.
2. **Group chat with a single operator.** If one person is the designated “operator” who relays questions, the session stays unified. Others can just observe.
3. **Discord channel.** Discord sessions are keyed by channel, so all users in the same channel share context. Use a dedicated channel for the shared conversation.
...
**Scenario:** You need to move or share only one specific profile — not the whole installation.
...
| **What’s included** | All profiles, global config, API keys, sessions | One profile: `SOUL.md`, memories, sessions, skills |
...
**Cause:** This usually happens when `~/.zshrc` (or `~/.bashrc`) has incorrect permissions, or the installer couldn't write to it cleanly. It's not a Hermes‑specific issue — it's a shell‑config permissions problem.
