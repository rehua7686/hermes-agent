...
| Register an audio-generation backend | `ctx.register_audio_gen_provider(provider)` — see [Audio Generation Provider Plugins](/developer-guide/audio-gen-provider-plugin) |
...
| An **audio-generation backend** (music, SFX, soundscapes — Lyria, GPT-Audio, …) | Backend plugin — `ctx.register_audio_gen_provider()` | [Audio Generation Provider Plugins](/developer-guide/audio-gen-provider-plugin) |
